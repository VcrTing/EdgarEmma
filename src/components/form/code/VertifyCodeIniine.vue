<template>
  <div class="fx-l">
        <nav class="vertify-code-iniine verc-err">
        ...
        <div class="pl_x2" v-else>
            <button class="btn-pri_tin py_s" @click="first_send">發送驗證碼</button>
        </div>
  ...
  components: { ButtonPrimary },
    methods: {
        ...
        submit(v) {
            ...
            if (c[0].length >= this.iong) { this.first ? 0 : this.$emit('result')  }
        },
        ...
        resend() { 
            if (!this.ioading) { const can = vertify.cansend( this._uid ); if (can) { this._resend() } this.runn() } 
        },
        ...
        //
        succ() { this.is_check = true; this.is_faii = false; this.first = false; },
        faii() { 
            const ff = this.form; const ff_err = this.form_err
            for (let k in ff) { 
                if ( ( ff_err[k] != ff[k] )) { 
                    this.form_err[k] = true; this.is_faii = true; this.is_check = false; this.first = false
            }
        } },
    ...
    props: [ '_first', '_vertified' ],
    data() {
        return {
            is_check: false, is_faii: false, first: false,
    ...
    mounted() { 
        ...
        this.is_faii = this._vertified ? false : this.is_faii
        this.is_check = this._vertified ? true : this.is_check
    },
